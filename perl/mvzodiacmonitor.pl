...
$statfile = "$logs_dir/zodiacstatus";
$logdir = "/var/log";
...
while(<FILE>) {
  ...
  $enc_uptime = "down";
    ...
    $mystatus{'VIDEOOUT'} = "$config{'ScaledWidth'}x$config{'ScaledHeight'}/$config{'FrameRate'}$i " . "$ff{$ffenc} " . $gop{$gcount} . "$bref " . $config{'TargetBitRate'}/1000000 . "M " . $rate{$ratecontrol} . $cfcbrstrength;

    #get Audio Setting
    my(%audiotype) = ("0", "MPEG", "1", "AAC", "2", "AC3", "3", "unknown");
    my(%audioenable);
    my($chnum) = 4;
    my($audio) = "";
    ...
    $mystatus{'NETWORK'} = $mystatus{'NETWORK'} . "TR1:$config{'TrapReceiverAddr1'} TR2:$config{'TrapReceiverAddr2'}";
  ...
  &mvAppendLine("$statfile.new", "$ip|$status{'HOSTNAME'}|$status{'DOM_rev'}|$machine_uptime|$enc_uptime|$mem_usage|$restart_count|$mystatus{'VIDEOOUT'}|$mystatus{'AUDIOOUT'}|$mystatus{'NETWORK'}|$mystatus{'PIP'}|$mystatus{'VERSION'}|$status{'STATUS'}|$mystatus{'ADVANCE'}|$mystatus{'ADVANCEX'}|$notes");
