...
sub mvGetConfig {
  my($myhost, $getwhat) = @_;
  my($mycmd) = "$rcmd $myhost \"/usr/local/hdenc/mvconfig --get $getwhat\"";
  my($rvalue) = &mvCmd($mycmd);
  my($temp);
...
sub mvConfigCmd {
  ...
  my($mycmd) = "$rcmd $myhost \"/usr/local/hdenc/mvconfig $mydefault\"";
  my($mykey, $myvalue);
  print ("$mycmd\n");
  my(@value) = `$mycmd`;
  for (@value) {
...
sub mvEncType {
  my($myhost) = @_;
  my($retval) = "hdenc";
  my($mycmd) = "$rcmd $myhost \"ls -lrt /var/log/hdenc.log\"";
  my($tmp) = &mvCmd($mycmd); 
  if($tmp =~ /mv/) {
    $retval = "mvenc"
  }
