...
$enchome = "/usr/local/zmux";
$enclog = "/var/log/zremux.log";
...
#Get zremux restart times
sub mvZremuxRestartCount {
  my($myhost) = @_;
  my($mycmd) = "$rcmd $myhost \"grep -c 'Starting encoder loop' /var/log/zremux.log\" ";
  my($rvalue) = &mvCmd($mycmd);
  chop($rvalue);
  chomp($rvalue);
  print("tt: $rvalue");
...
sub mvServiceStatus {
  my($myhost) = @_;
  my($mycmd) = "$rcmd $myhost \"/sbin/service zremux status\"";
  my($rvalue) = &mvCmd($mycmd);
  my($ret) = "down";
  if ($rvalue =~ /running/) {
      $ret = "up";
