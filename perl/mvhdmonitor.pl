...
require("$basedir/mvhdlib.pl");

use vars qw($lfile $Statfile $ip $machine_uptime $enc_uptime $mem_usage $disk_usage $servicestatus $logdir);
use vars qw(%config %status %mystatus $key $ip $notes $enc_type);


$lfile = "$logs_dir/hdhostname.lst";
$statfile = "$logs_dir/hdstatus";
$logdir = "/var/log";
...
while(<FILE>) {
  ...
  &mvAppendLine("$statfile.new", "$ip|$status{'HOSTNAME'}|$machine_uptime|$enc_uptime|$mem_usage|$disk_usage|$mystatus{'VIDEOOUT'}|$mystatus{'AUDIOOUT'}|$mystatus{'NETWORK'}|$mystatus{'PIP'}|$mystatus{'VERSION'}|$status{'STATUS'}|$mystatus{'ADVANCE'}|$mystatus{'ADVANCEX'}|$notes");
