...
require("$basedir/mvstatmuxlib.pl");

use vars qw($lfile $Statfile $ip $machine_uptime $enc_uptime $mem_usage $restart_count $temp_sensors $servicestatus $logdir);
use vars qw(%config %status %mystatus $key $ip $notes $service_name $enc_type);


$lfile = "$logs_dir/statmuxhostname.lst";
$statfile = "$logs_dir/statmuxstatus";
$logdir = "/var/log";
...
while(<FILE>) {
  ...
  $enc_uptime = "down";
    ...
    if ($servicestatus eq "up") {
      ...
      $restart_count = &mvZremuxRestartCount($ip);
      $time = `date +\'%d-%m-%y %H:%M:%S\'`;
      chop($time);
      chomp($time);
      &mvAppendLine("$logs_dir/statmux/$ip", "$time $mem_usage\tzremux_uptime=$enc_uptime");
    }
    ...
    if ($config{'StatmuxEnable'} == "1") {
      $mystatus{'NETWORK'} = $mystatus{'NETWORK'} . "PMUX:" . "$config{'StatmuxPrimaryAddress'} " . "SMUX:" . "$config{'StatmuxSecondaryAddress'} ";
    }
  ...
  &mvAppendLine("$statfile.new", "$ip|$status{'HOSTNAME'}|$machine_uptime|$enc_uptime|$mem_usage|$restart_count|$mystatus{'VIDEOOUT'}|$mystatus{'AUDIOOUT'}|$mystatus{'NETWORK'}|$mystatus{'PIP'}|$mystatus{'VERSION'}|$status{'STATUS'}|$mystatus{'ADVANCE'}|$mystatus{'ADVANCEX'}|$notes");
