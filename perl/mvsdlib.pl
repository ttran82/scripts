...
use vars qw($logs_dir $scripts_dir $rcmd $rscp $logfile);
...
$rcmd = "$scripts_dir/mvsdrexec.exp";
$rscp = "$scripts_dir/mvsdrscp.exp";
...
sub mvServiceStatus {
  my($myhost) = @_;
  my($mycmd) = "$rcmd $myhost \"service mvenc status\"";
  my($rvalue) = &mvCmd($mycmd);
  my($ret) = "down";
  if ($rvalue =~ /running/) {
      $ret = "up";
...
sub mvEncUptime {
  my($myhost) = @_;
  my($curlog) = "$logs_dir/$myhost.current";
  my($rlog) = "/var/log/mvenc.log"; 
  my($curtime, $oldtime, $difftime, $mycmd);
  $curtime = &mvEncCurTime($myhost);
  print "curtime: $curtime\n";

  open(FD, "<$curlog") or die("Cannot open file $curlog.\n");
  while(<FD>) {
    if (/Starting application at  (\d+):(\d+):\d+ (\d+)\/(\d+)\/(\d+)/) {
       #print $_;
       #print("0 $2 $1 $4 $3 $5\n"); 
       $oldtime = timelocal(0, $2, $1, $4, $3-1, $5); 
    }
    if (/Restart at  (\d+):(\d+) (\d+)\/(\d+)\/(\d+)/) {
       #print $_;
       #print("0 $2 $1 $4 $3 $5\n");
       $oldtime = timelocal(0, $2, $1, $4, $3-1, $5);
    }
...
sub mvGetConfig {
  my($myhost, $getwhat) = @_;
  my($mycmd) = "$rcmd $myhost \"/usr/local/mvenc/mvconfig --get $getwhat\"";
  my($rvalue) = &mvCmd($mycmd);
  my($temp);
...
sub mvConfigStatus {
  my($myhost) = @_;
  my($mycmd) = "$rcmd $myhost \"/usr/local/mvenc/mvconfig --status\"";
  my(@rvalue) = `$mycmd`;
  ...
  my($mycmd) = "$rcmd $myhost \"/usr/local/mvenc/mvconfig --get InterfaceStandby\"";
  $myvalue = &mvCmd($mycmd);
...
sub mvConfigCmd {
  ...
  my($mycmd) = "$rcmd $myhost \"/usr/local/mvenc/mvconfig $mydefault\"";
  my($mykey, $myvalue);
  print ("$mycmd\n");
  my(@value) = `$mycmd`;
  for (@value) {
