...
def Set_default_arg(myarg, myvalue):
    if not Get_arg(myarg):
        Set_arg(myarg, myvalue)
        Print_debug(myarg + ' is not given.  Use default value: ' + myvalue)
...
#Any global varibles
if Get_arg('KleverTestScriptsDir'):
    sikuliScriptDir = Get_arg('KleverTestScriptsDir') + '\\sikuli\\'
else:
...
def Launch_winapp(texttosearch, waittime=2):
